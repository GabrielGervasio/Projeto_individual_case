...
export async function insertFuncionario(req, res) {
  ...
  openDb().then((db) => {
    db.run(
      'INSERT INTO Funcionarios (nome, sobrenome, cargo, descricao) VALUES (?, ?, ?, ?)',
      [funcionario.nome, funcionario.sobrenome, funcionario.cargo, funcionario.descricao]
    );
  });
  res.json({
    statusCode: 200,
...
export async function updateFuncionario(req, res) {
  let funcionario = req.body;
  openDb().then((db) => {
    db.run('UPDATE Funcionarios SET nome=?, sobrenome=?, cargo=?, descricao=? WHERE id=?', [
      funcionario.nome,
